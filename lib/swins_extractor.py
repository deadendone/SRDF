...
def load_checkpoint(model, checkpoint_path, map_location='cpu'):
    """
    替代mmcv的load_checkpoint，仅保留核心权重加载逻辑
    """
    # 加载权重文件（兼容.pth/.pkl格式）
    checkpoint = torch.load(checkpoint_path, map_location=map_location)
    # 适配不同格式的权重（部分权重文件包含state_dict键）
    if 'state_dict' in checkpoint:
        state_dict = checkpoint['state_dict']
    else:
        state_dict = checkpoint
    # 加载权重（strict=False 忽略不匹配的层，避免报错）
...
from lib.builder import BACKBONES
...
class FuseLayers(nn.Module):
    ...
    #     self.conv3 = self.Conv1(self.channel_nums[3] + self.channel_nums[0], 512)
    """多尺度特征融合层（适配Swin-Base 512输入）"""
    def __init__(self, in_dims=[128, 256, 512, 1024], out_dim=512):
        super().__init__()
        ...
        self.final_conv = nn.Conv2d(len(in_dims) * out_dim, out_dim, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, features):
        """
        Args:
            features: 列表，元素为 (B, C, h, w)（Swin各stage输出）
        Returns:
            fused_feat: 融合后的特征 (B, out_dim, 128, 128)
        """
        fused = []
        # 统一上采样到第一个stage的分辨率（512/4=128）
        ...
        for i, feat in enumerate(features):
            ...
            # 维度映射到out_dim
            fused.append(self.fuse_convs[i](feat))

        # 拼接所有尺度特征并融合
        fused = torch.cat(fused, dim=1)
        fused_feat = self.relu(self.bn(self.final_conv(fused)))
        return fused_feat
...
# swins_extractor.py 末尾新增
class FuseLayers(nn.Module):
    """多尺度特征融合层（适配HGINet的FuseLayers调用）"""
    def __init__(self, in_dims=[96, 192, 384, 768], out_dim=256):
        super().__init__()
        self.fuse_convs = nn.ModuleList()
        for dim in in_dims:
            self.fuse_convs.append(
    ...
    def forward(self, features):
        """
        Args:
            features: 列表，包含Swin Transformer各阶段输出的特征图 [(B, C1, H1, W1), (B, C2, H2, W2), ...]
        Returns:
            fused_feat: 融合后的特征图 (B, out_dim, H, W)
        """
        fused = []
        for i, feat in enumerate(features):
            # 上采样到第一阶段特征的分辨率
