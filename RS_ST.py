...
###JL数据集
# num_classes = 6
# ST_COLORMAP = [[255,255,255], [0,128,0], [255,0,0], [0,255,0], [128,0,0], [128,128,128]]
# ST_CLASSES = ['unchanged', 'cropland', 'road', 'forest', 'building','ground']
#
...
MEAN_B = np.array([111.07, 114.04, 118.18])
STD_B  = np.array([49.41,  47.01,  47.94])
# root = r'/home/zwenbo/SCD-DataSets/data_10/data_10'
root = r'/home/zwenbo/SCD-DataSets/SCD-DataSets'

##HRSCD
# num_classes = 6
# # ST_COLORMAP = [[128,128,128], [255,0,0], [250,250,150], [150,250,150], [0,255,0], [0,0,255]]  ##原始的
# ST_COLORMAP = [[255,255,255], [128,128,128], [0,128,0], [0,255,0], [150,250,150], [0,0,255]]
...
# STD_B  = np.array([49.41,  47.01,  47.94])
#
# root = r'E:\Change-Model\HRSCD\HRSCD_D35\D35\D35'

##FZ dataset
# num_classes = 7
# ST_COLORMAP = [[255,255,255], [0,128,0],[0,255,0],[0,0,255],[128,0,0],[255,0,0],[128,128,128]]
# ST_CLASSES = ['unchanged', 'cropland','tree','water','building','road','ground']
#
# MEAN_A = np.array([113.40, 114.08, 116.45])
# STD_A  = np.array([48.30,  46.27,  48.14])
# MEAN_B = np.array([111.07, 114.04, 118.18])
...
def read_RSimages(mode, rescale=False):
    ...
    for it in data_list:
        ...
        if (it[-4:]=='.tif') or (it[-4:]=='.png'):
            ...
            label_A_path = os.path.join(label_A_dir, it[:-4]+'.png')
            label_B_path = os.path.join(label_B_dir, it[:-4]+'.png')
            label_C_path = os.path.join(label_contour_dir, it[:-4]+'.png')
    ...
    return imgs_list_A, imgs_list_B, labels_A, labels_B, labels_C

class Data(data.Dataset):
    def __init__(self, mode, random_flip = False):
        self.random_flip = random_flip
        self.imgs_list_A, self.imgs_list_B, self.labels_A, self.labels_B, self.labels_C = read_RSimages(mode)
    ...
    def __getitem__(self, idx):
        ...
        # label_A = np.array(label_A, dtype='uint8') ##HRSCD数据集要加
        label_B = self.labels_B[idx]
        label_C = self.labels_C[idx]

        # label_B = np.array(label_B, dtype='uint8')  ###HRSCD数据集要加
        if self.random_flip:
            img_A, img_B, label_A, label_B, label_C = transform.rand_rot90_flip_MCD(img_A, img_B, label_A, label_B, label_C)
        return to_tensor(img_A), to_tensor(img_B), torch.from_numpy(label_A), torch.from_numpy(label_B), torch.from_numpy(label_C)
...
class Data_test(data.Dataset):
    def __init__(self, test_dir):
        ...
        # c_dir = os.path.join(test_dir, 'edge_prior')
        c_dir = os.path.join(test_dir, 'boundary')
    ...
    def __getitem__(self, idx):
        ...
        boundaries = self.contours[idx]
        boundaries = boundaries/255

        img_A = normalize_image(img_A, 'A')
        img_B = normalize_image(img_B, 'B')
        # 关键修改：添加返回 boundaries（作为第三个值）

        # return to_tensor(img_A), to_tensor(img_B)
        return to_tensor(img_A), to_tensor(img_B), torch.from_numpy(boundaries)
