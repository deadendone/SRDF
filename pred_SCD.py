...
DATA_NAME = 'SRDF_HGM+DE+SR_SECOND_100e_316'
#################################
...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--test_dir', required=False, default=r'/home/zwenbo/SCD-DataSets/SCD-DataSets/test', help='directory to test images')
        parser.add_argument('--pred_dir', required=False, default=r'/home/zwenbo/BGSNet-main/BGSNet-main/SECOND_result/BGSNet_v3/BGSNet_HGM+DE+SR_SECOND_100e_316_pred_test', help='directory to output masks')
        # parser.add_argument('--chkpt_path', required=False, default=r'/home/zwenbo/BGSNet-main/BGSNet-main/checkpoints/HRSCD_100e_119/BGSNet_42e_mIoU48.74_Sek64.08_Fscd86.64_OA84.04.pth')
        parser.add_argument('--chkpt_path', required=False,
                            default=r'/home/zwenbo/BGSNet-main/BGSNet-main/checkpoints/train_v3_HGM+DE+SR_SECOND_100e_316/BGSNetv3_HGM+DE+SR_gpu1/BGSNetv3_HGM+DE+SR_gpu1_73e_mIoU70.31_Sek20.36_Fscd60.25_OA85.29.pth')
                            # default=r'/home/zwenbo/BGSNet-main/BGSNet-main/checkpoints/SECOND_200e_124/BGSNet_62e_mIoU69.91_Sek19.77_Fscd59.54_OA84.93.pth')
...
def main():
    ...
    # net = Net(64, num_classes=RS.num_classes, pretrained_path='/HOME/pretrained/pvt_v2_b2.pth').cuda()
    # net = Net(64, num_classes=RS.num_classes, pretrained_path='/home/zwenbo/BGSNet-main/BGSNet-main/checkpoints/SECOND_e100_1230/BGSNet_86e_mIoU67.55_Sek16.38_Fscd55.62_OA82.92.pth').cuda()
    net = Net(64, num_classes=RS.num_classes,
              pretrained_path='/home/zwenbo/BGSNet-main/BGSNet-main/checkpoints/train_v3_HGM+DE+SR_SECOND_100e_316/BGSNetv3_HGM+DE+SR_gpu1/BGSNetv3_HGM+DE+SR_gpu1_73e_mIoU70.31_Sek20.36_Fscd60.25_OA85.29.pth',
              use_hgm=True, use_de=True, use_sr=True).cuda()
    # net = Net(64, num_classes=RS.num_classes).cuda()  # 不指定pretrained_path，使用默认的None
...
def predict(net, pred_set, pred_loader, pred_dir, flip=False, index_map=False, intermediate=False):
    ...
    pred_boundary_dir = os.path.join(pred_dir, 'boundary')
    ...
    if not os.path.exists(pred_boundary_dir): os.makedirs(pred_boundary_dir)
    if index_map:
    ...
    for vi, data in enumerate(pred_loader):
        imgs_A, imgs_B, label_contour = data
        #imgs = torch.cat([imgs_A, imgs_B], 1)
        imgs_A = imgs_A.cuda().float()
        imgs_B = imgs_B.cuda().float()
        label_contour = label_contour.cuda().float()
        mask_name = pred_set.get_mask_name(vi)
        with torch.no_grad(): 
            out_change, outputs_A, outputs_B, out_bd = net(imgs_A, imgs_B)#,aux
            out_change = F.sigmoid(out_change)
            out_bds = F.sigmoid(out_bd)
        if flip:
        ...
        #
        out_boundary = (out_bds.cpu().detach()>0.5).squeeze()
        ##
        ...
        pred_Boundary_path = os.path.join(pred_boundary_dir, mask_name)

        io.imsave(pred_A_path, RS.Index2Color(pred_A))
        io.imsave(pred_B_path, RS.Index2Color(pred_B))
        #保存边界
        io.imsave(pred_Boundary_path, np.array((out_boundary.numpy()) * 255, dtype='uint8'))
